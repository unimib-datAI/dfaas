...
type Configuration struct {
	...
	// FaaS platform connection info (host:port). Used for all supported
	// platforms.
	...
	// FaaS platform backend. Supported: "openfaas" (default), "openwhisk".
	FaaSPlatform string `mapstructure:"AGENT_FAAS_PLATFORM"`

	// OpenWhiskNamespace is the OpenWhisk namespace to query actions from. Only
	// used when FaaSPlatform is "openwhisk". Defaults to "guest".
	OpenWhiskNamespace string `mapstructure:"AGENT_OPENWHISK_NAMESPACE"`

	// OpenWhiskAPIKey is the OpenWhisk API key in "uuid:key" format. Only used
	// when FaaSPlatform is "openwhisk".
