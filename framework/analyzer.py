...
def calculate_rates_globally(
    function_rate_x_node, df_presence, config_file, strategy_name
):
    ...
    for index, row in df_presence.iterrows():
        ...
        rate_only_present_functions = (
            function_rate_x_node.loc[index] * df_presence.loc[index]
        )
        # Obtains data in group format
        features_data = model_manager.transform_functions_in_groups(
            rate_only_present_functions
        )
        ...
        while overload == 1 or (
            strategy_name == "power_saving_strategy"
            and node_power_consumption > config_manager.MAX_POWER_USAGE[node_type]
        ):
            # Calculate the the percentage value for each element
            percentage_values = [
                round(element * load_discard_percentage)
                for element in rate_only_present_functions
            ]
            ...
            for i in range(0, len(rate_only_present_functions)):
                if rate_only_present_functions[i] > 0:
                    ...
                    func_reject_num[rate_only_present_functions.index[i]] += (
                        rate_to_remove
                    )
    ...
    for func in func_reject_num:
        ...
        success_rate[func] = (
            (success_number / total_invoc_rate_x_function[func])
            if total_invoc_rate_x_function[func] > 0
            else 1.0
        )
...
def export_for_minute_overloaded_nodes(overloaded_for_algo):
    ...
    df = pd.DataFrame(
        data=overloaded_for_algo,
        index=[i for i in range(0, config_manager.SIMULATION_MINUTES)],
    )
    ...
    plt.savefig(
        create_path_if_not_exists(config_manager.ANALYZER_OUTPUT_PATH).joinpath(
            "comparison_nodes.png"
        )
    )
...
def export_for_minute_rates(func, rates):
    ...
    plt.figure(figsize=(20, 10))
    plt.title(
        "Success rate for function {} during {} minutes of experiment".format(
            func, config_manager.SIMULATION_MINUTES
        )
    )
    plt.xlabel("Minute")
    ...
    for i, column in enumerate(df.columns):
        ...
        plt.plot(
            df.index,
            df[column],
            label="Success rate for {}".format(column),
            linestyle=linestyle,
            color=color,
        )
...
def main():
    ...
    for algo in config_manager.STRATEGIES:
        ...
        for minute in range(0, config_manager.SIMULATION_MINUTES):
            ...
            sr, rr, rn, an, mc, on = calculate_rates_globally(
                function_rate_x_node, df_presence, config_file, algo
            )
        ...
        mean_node_consumption = sum(x_node_power_consumption) / len(
            x_node_power_consumption
        )
        print(
            "     > Mean node power consumption: {:0.2f}%".format(mean_node_consumption)
        )

        # Max node consumption
        max_node_consumption = max(x_node_max_power_consumption)
        print(
            "     > Max node power consumption: {:0.2f}%".format(max_node_consumption)
        )
        ...
        mean_success_rate_stress_period = (
            np.mean(
                [np.mean(srates[low:high]) for k, srates in x_func_success_rate.items()]
            )
            * 100
        )
        print(
            "         > Mean success rate during stress period: {:0.2f}%".format(
                mean_success_rate_stress_period
            )
        )
        ...
        flat_list = flatten(
            [item[low:high] for item in list(x_func_success_rate.values())]
        )
        success_rate_stress_period_median = np.median(flat_list) * 100
        print(
            "         > Success rate median during stress period: {:0.2f}%".format(
