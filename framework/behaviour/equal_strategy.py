...
class EqualStrategy(Strategy):
    ...
    def loop(self) -> dict:
        ...
        original_requests_groups["node_type"] = (
            self._config_manager.NODES_TYPES_IN_MODELS[self._config_json["node_type"]]
        )
        ...
        while overload == 1:
            ...
            for i in range(0, len(node_functions)):
                if remained_requests_functions[node_functions[i]] > 0:
                    ...
                    load_to_discard = math.ceil(
                        load_discard_percentage
                        * remained_requests_functions[node_functions[i]]
                    )
        ...
        for func in node_functions:
            ...
            rem_mant_request_function[func]["removed"] = (
                original_requests_functions[func] - remained_requests_functions[func]
            )
            rem_mant_request_function[func]["mantained"] = remained_requests_functions[
                func
            ]
        ...
        for fun in rem_mant_request_function.keys():
            ...
            tot_n_of_req = (
                rem_mant_request_function[fun]["removed"]
                + rem_mant_request_function[fun]["mantained"]
            )
            # Use divmod to get the quotient and remainder
            quotient, remainder = divmod(
                rem_mant_request_function[fun]["removed"], n_of_neigh
            )
            for node in self._config_json["neighbours"] + [self._id]:
                if tot_n_of_req == 0:
                    ...
                    if node != self._id:
                        weights[fun][node] = round(
                            (quotient + remainder) / tot_n_of_req, 4
                        )
                        remainder = 0
                    else:
                        weights[fun][node] = round(
                            rem_mant_request_function[fun]["mantained"] / tot_n_of_req,
                            4,
                        )
