...
class NodeMarginStrategy(Strategy):
            ...
            if node_infos["node_metrics"]["overloaded_node"] == 0:
                ...
                if neigh_num == 0:
                    ...
                    for (
                        metric,
                        values,
                    ) in self._config_manager.MAX_RESOURCES_USAGE.items():
                        ...
                        if node_infos["node_metrics"][metric] > values[node_type]:
                            ...
                            usage_percentage[metric] = (
                                node_infos["node_metrics"][metric] * 100
                            ) / values[node_type]
    ...
    def _calculate_weights(self):
        ...
        original_requests_groups["node_type"] = (
            self._config_manager.NODES_TYPES_IN_MODELS[
                self._config_json[self._id]["node_type"]
            ]
        )
            ...
            if not zero_neigh_available:
                ...
                if remained_requests_functions[func_to] > 0:
                    ...
                    original_node_to_features["node_type"] = (
                        self._config_manager.NODES_TYPES_IN_MODELS[
                            self._margin_percentage_out[node_to]["node_type"]
                        ]
                    )
                    ...
                    # Calculate the percentage usage for each node metric
                    original_node_to_percentage = self.__calculate_usage_node(
                        original_node_to_predictions,
                        self._margin_percentage_out[node_to]["node_type"],
                    )
                    ...
                    # Calculate the percentage usage for each node metric
                    node_to_percentage = self.__calculate_usage_node(
                        node_to_predictions,
                        self._margin_percentage_out[node_to]["node_type"],
                    )
