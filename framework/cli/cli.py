...
def analyzer_and_simulator_arguments():
    ...
    if (
        args.modeltype != "regression"
        and args.modeltype != "quantile005"
        and args.modeltype != "quantile095"
    ):
        raise parser.error(
            'Model type can only be "regression" "quantile005" or "quantile095"'
        )
    return args
...
def simulation_controller_arguments():
    ...
    if args.seed != -1 and args.expnum > 1:
        raise parser.error(
            "Seed cannot be setted when experiments number is greater than 1"
        )
    ...
    if args.overloaded is not None:
        for rate in args.overloaded:
            if rate < 0 or rate > 100:
                raise parser.error(
                    "Overloaded percentage must be integer, grater eq than 0 and lower eq than 100"
                )
    return args
