...
class ConfigManager(object):
    ...
    OUTPUT_INSTANCE_JSON_FILE_PATH = OUTPUT_INSTANCE_PATH.joinpath(
        "instance.json"
    )  # Path for output instance json file
    OUTPUT_INSTANCE_GRAPH_FILE_PATH = OUTPUT_INSTANCE_PATH.joinpath("graph.png")
    DATA_DIR = simulation_dir.joinpath(
        "data"
    )  # Directory that contains experiment files

    NODES_TYPES = ["HEAVY", "MID", "LIGHT"]  # Nodes types used in experiments
    NODES_TYPE_PROBABILITY_DISTRIBUTION = [0.2, 0.3, 0.5]
    NODES_TYPES_IN_MODELS = {
        "HEAVY": 0,
        "MID": 1,
        "LIGHT": 2,
    }  # Association of the nodes types with the values used in the models
    PREDICTED_METRICS = [
        "cpu_usage_node",
        "overloaded_node",
        "power_usage_node",
        "ram_usage_node",
    ]  # Metrics predicted by the forecaster
    ...
    MAX_RESOURCES_USAGE = {
        "cpu_usage_node": {"HEAVY": 460, "MID": 290, "LIGHT": 150},
    ...
    MAX_POWER_USAGE = {
        node_type: value * 0.70
        for node_type, value in MAX_RESOURCES_USAGE["power_usage_node"].items()
    }
    ...
    STRATEGIES = [
        "base_strategy",
        "equal_strategy",
        "node_margin_strategy",
        "power_saving_strategy",
    ]
    ...
    SIMULATION_COMPLETE_CONFIGURATION_OUTPUT_PATH = SIMULATION_OUTPUT_DIR.joinpath(
        "minute_config"
    )
    ...
    SIMULATION_CONTROLLER_OUTPUT_PATH = output_dir.joinpath(
        "simulation_controller_output"
    )
    SIMULATION_CONTROLLER_OUTPUT_FILE = SIMULATION_CONTROLLER_OUTPUT_PATH.joinpath(
        "experiment_results.csv"
    )
    SIMULATION_CONTROLLER_OUTPUT_MEAN_FILE = SIMULATION_CONTROLLER_OUTPUT_PATH.joinpath(
        "experiment_mean_results.csv"
    )
