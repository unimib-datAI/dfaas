...
def generate_configs(nodes_number, overloaded_max_percentage, seed):
    ...
    for choice in choices:
        ...
        groups = net_gen.choice(
            list(config_manager.GROUPS.keys()), node_groups_number, replace=False
        )
        ...
        for group in groups:
            ...
            num_elements_to_choose = net_gen.integers(
                1, len(config_manager.GROUPS[group]) + 1
            )
            # TODO: puoi usare il choice del net_gen al posto del sample
            functions_groups[group] = net_gen.choice(
                config_manager.GROUPS[group], num_elements_to_choose, replace=False
            )

        max_rates_o = {
            "HEAVY": {"LOW_USAGE": 400, "HIGH_USAGE": 90, "MEDIUM_USAGE": 200},
        ...
        max_rates_u = {
            key1: {key2: value * 0.15 for key2, value in inner_dict.items()}
            for key1, inner_dict in max_rates_o.items()
        }
        ...
        for instant in range(0, config_manager.SIMULATION_MINUTES):
            ...
            for group in groups:
                ...
                if instant < maximum_rate_instant:
                    group_load = load_gen.integers(
                        math.floor(
                            instant * (max_rates[choice][group] / maximum_rate_instant)
                        ),
                        math.ceil(
                            (instant + 1)
                            * (max_rates[choice][group] / maximum_rate_instant)
                        ),
                        endpoint=True,
                    )
                else:
                ...
                loads = [
                    load_gen.integers(0, group_load, endpoint=True)
                    for _ in range(len(functions_groups[group]))
                ]
...
def plot_node_load(node_config, groups):
    ...
    plt.title(
        "Rate for each group in a {} node during {} minutes of experiment".format(
            node_config["node_type"], config_manager.SIMULATION_MINUTES
        )
    )
    plt.xlabel("Minute")
    ...
    # Create DataFrame
    df = pd.DataFrame(
        {"LOW_USAGE": low_usage, "MEDIUM_USAGE": medium_usage, "HIGH_USAGE": high_usage}
    )
...
def main():
    ...
    instance_json = build_output_json(
        seed, nodes_num, probability, overloaded_max_percentage, G
    )
