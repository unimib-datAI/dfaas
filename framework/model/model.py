...
class Model:
    ...
    def __init__(self, metric, model_type):
        ...
        self.model = joblib.load(
            self.__config_manager.MODEL_BASE_PATH
            + metric
            + "/"
            + model_type
            + "/model.joblib"
        ...
        if "overloaded" in metric:
            self.features_scaler = joblib.load(
                self.__config_manager.SCALER_BASE_PATH
                + "scaler_x/"
                + metric
                + ".joblib"
            )
            ...
            self.features_scaler = joblib.load(
                self.__config_manager.SCALER_BASE_PATH + "scaler_x/features.joblib"
            )
    ...
    def predict(self, input_data):
        ...
        input_data_scaled = pd.DataFrame(
            input_data_scaled,
            columns=[*self.__config_manager.GROUPS_COLUMNS_NAMES, "node_type"],
        )
        scaled_predictions = self.model.predict(input_data_scaled)
        if "overloaded" in self.metric:
            return scaled_predictions
        else:
            original_predictions = self.target_scaler.inverse_transform(
