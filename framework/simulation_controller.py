...
def main():
    ...
    for i in range(1, n_of_experiments + 1):
        ...
        for percentage in max_percentage_values:
            ...
            df = pd.read_csv(
                config_manager.INDEX_COMPARISON_FILE,
                delimiter=",",
                header=0,
                index_col=0,
            )
    ...
    final_df.to_csv(
        config_manager.SIMULATION_CONTROLLER_OUTPUT_FILE,
        sep=",",
        encoding="utf-8",
        index=False,
    )
    ...
    mean_df.reset_index(
        drop=False, inplace=True, names=["Max overloaded percentage", "Strategy"]
    )
    mean_df.insert(0, "N of experiments", pd.Series([n_of_experiments], index=[0]))

    mean_df.to_csv(
        config_manager.SIMULATION_CONTROLLER_OUTPUT_MEAN_FILE,
        sep=",",
        encoding="utf-8",
        index=False,
    )
