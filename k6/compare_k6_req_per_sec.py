...
def main():
    ...
    target = target.select([
        pl.col(STAGE_COL),
        pl.col(TARGET_COL).alias("target_avg_reqs_per_sec")
    ])
    source = source.select([
        pl.col(STAGE_COL),
        pl.col(SOURCE_COL).alias("calculated_avg_reqs_per_sec")
    ])
    ...
    result = merged.with_columns([
        (pl.col("calculated_avg_reqs_per_sec") - pl.col("target_avg_reqs_per_sec")).alias("diff"),
    ]).with_columns([
        pl.col("diff").abs().alias("diff_abs")
    ]).select([
        STAGE_COL,
