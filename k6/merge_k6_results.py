...
def ensure_k6_processed(node_dir, rl_strategy=False):
...
def merge_experiment(exp_dir, rl_strategy=False):
    ...
    for node_dir in sorted(k6_dir.glob("node_*")):
        ...
        csv_file = ensure_k6_processed(node_dir, rl_strategy)
...
located. In this case, you may provide the --rl-strategy flag if the experiment
used the RL Agent strategy.
""",
    )
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        # Must pass --rl-strategy flag to the merge_experiment function.
        executor.map(lambda exp: merge_experiment(exp, args.rl_strategy), experiments)
