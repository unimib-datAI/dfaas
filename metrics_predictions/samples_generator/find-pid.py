...
def main():
    ...
    for row in rows:
        ...
        for function_name in functions:
            if data[1].__contains__(function_name) or (
                data[1].__contains__("ghcr.io/openfaas/")
                and str(data[-1:]).__contains__(function_name)
            ):
    ...
    for function_name in functions:
        if len(containers_id[function_name]) > 0:
            ...
            for container in containers_id[function_name]:
                ...
                functions_pids[function_name].append(
                    "".join(re.findall("\d+", str(temp.communicate())))
                )
        else:
            raise Exception(
                "Error! Container ID not found for function ", function_name
            )
