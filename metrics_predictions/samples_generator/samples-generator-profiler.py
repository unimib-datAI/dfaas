    ...
    for function in FUNCTION_NAMES:
        ...
        for func in ALL_FUNCTIONS:
            if func.__contains__("/"):
                subprocess.call(['faas-cli', 'remove', func.split("/")[1], '--gateway', OPENFAAS_SERVICE_IP], shell=False)
            else:
                subprocess.call(['faas-cli', 'remove', func, '--gateway', OPENFAAS_SERVICE_IP], shell=False)
        
        # Wait until the functions are successfully removed.
        time.sleep(40)

        # Deploy the current function
        subprocess.call(['./deploy_functions.sh'] + [str(MAX_RATE)] + [function], shell=False)
        ...
        RESULT_FILE_NAME = f'../output/generator/results-{current_datetime}-{function}.csv'
        ...
        for config in function_with_rate:
            ...
            try:
                ...
                for j in range(0, MAX_ITERATION_PER_CONFIG):
                    ...
                    function_name = config[0]; invocation_rate = config[1]
