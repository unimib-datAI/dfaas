...
def main():
    ...
    for i in range(0, config_manager.NUMBER_OF_SIMULATION_EXECUTION):
        ...
        df = pd.read_csv(
            config_manager.INDEX_COMPARISON_FILE, delimiter="\t", header=0, index_col=0
        )
    ...
    final_df.to_csv(
        config_manager.SIMULATION_CONTROLLER_OUTPUT_FILE, sep="\t", encoding="utf-8"
    )
    ...
    with open(
        dir_path.joinpath(
            config_manager.SIMULATION_CONTROLLER_ARCHIVE_COMPARISON_FILE_NAME
        ),
        "w",
    ) as f:
